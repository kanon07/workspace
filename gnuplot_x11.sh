...
case "$expmode" in
    ...
    th1=`find ${sender1dir} -name sender1_iperf*.txt`
...
srtt="set terminal x11 5 title 'sRTT, mode' size $size position $srtposi; set grid; set xlabel 'Time [s]'; set ylabel 'RTT [ms]'; plot $rttrange $range '$kernel1' using 1:(\$26/1000.0) with lines lc 7 title 'sRTT' ; replot '$kernel1' using 1:(\$14/1000.0) with linespoints title 'RTprop'; reset"
